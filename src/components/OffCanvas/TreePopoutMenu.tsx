...
const TreePopoutMenu = ({diagramRef}: {diagramRef: React.RefObject<go.Diagram>}) => {
    ...
    return (
        <TPS.OffCanvasWrapper show={show} onHide={handleClose}>
            ...
            <Button variant="secondary" onClick={() => {
                console.log(diagramRef.current?.model.nodeDataArray)
                const diaToSave = convertDiagramData(diagramRef.current?.model.nodeDataArray, diagramRef.current?.model.linkDataArray)
